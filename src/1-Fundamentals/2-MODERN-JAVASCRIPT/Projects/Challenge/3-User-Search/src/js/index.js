(async() => {
    ...
    function start() {
        ...
        searchInput.addEventListener('keyup', ({ key, target }) => {
            ...
            searchInput.value.length > 0 && handleSubmit ? searchButton.classList.remove("disabled") : searchButton.classList.add("disabled")
    ...
    async function fetchUsers() {
        ...
        const filteredInfo = arr.map(({ name: { first, last }, dob: { age }, gender, picture: { large } }) => {
            return {
                name: `${first} ${last}`,
                gender,
                age,
    ...
    function search(users) {
        ...
        results.length === 0 ? results = [{ name: null, gender: null, age: null, photo: null }] : results
        cleanInput()
    ...
    function renderResults(users) {
        ...
        users.forEach(({ name, age, photo }) => {
            ...
            age === null ? userInfor.textContent = `${name}` : userInfor.textContent = `${name}, ${age} anos`
    ...
    function renderSummary(users) {
        ...
        let femaleCount = users.filter(({ gender }) => gender === "female").length
        ...
        averageAgesHTML.textContent = `Média das idades: ${averageAges.toFixed(2)}`
        ...
        ChildList.length > 0 && ChildList[0].id === "summaryInfo" ? content.removeChild(content.childNodes.item(0)) : null
    ...
    function changeTheme() {
        ...
        actualTheme === "light-theme" ? newTheme = "dark-theme" : newTheme = "light-theme"
        newTheme === "dark-theme" ? themeText = "🌚 Tema Escuro" : themeText = "🌞 Tema Claro"
